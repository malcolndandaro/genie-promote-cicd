...
**Dev also needs the engine app assembled.** The `genie-engine-api` app (dev-only)
deploys from `build/engine_app/` (gitignored, generated by `scripts/build_engine_app.sh`).
**Before any `bundle deploy/validate -t dev`, run `bash scripts/build_engine_app.sh`** —
...
`stat build/engine_app: no such file or directory` until the assembly runs.
