...
10. Verifique que o deploy completa todas as etapas: preflight, `bundle deploy`, resolução do Space,
    ...
    que ela declara `certified`, mas o Databricks não expõe aqui uma prova estável e não mutante de
    `ASSIGN`: uma ausência desse privilégio falha honestamente em `certify_space`. A resolução
    pós-deploy tolera por cerca de 60 segundos a consistência eventual da listagem do Genie; títulos
    duplicados ainda falham imediatamente, sem adivinhar um ID.
...
| `bundle deploy` termina, mas `resolve_space` não encontra o título | A listagem do Genie pode ficar brevemente atrás do recurso recém-criado; o resolver tenta novamente por cerca de 60 segundos. Se ainda falhar, compare o `.title` renderizado com o título ao vivo e procure títulos duplicados ou falta de visibilidade do SP. |
